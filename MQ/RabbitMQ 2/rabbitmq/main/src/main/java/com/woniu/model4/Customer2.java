...
public class Customer2 {
    ...
    private static final String QUEEUNAME = "MQ81";
    public static void main(String[] args) throws Exception {
        Connection conn = ConnectionUtil.getConnection();

        Channel channel = conn.createChannel();
        channel.exchangeDeclare(EXECHANGNAME, "direct");

        //声明消息队列
        channel.queueDeclare(QUEEUNAME, true, false, false, null);
        //把消息队列绑定到交换机上
        channel.queueBind(QUEEUNAME, EXECHANGNAME, "error");
        channel.queueBind(QUEEUNAME, EXECHANGNAME, "debug");
        channel.basicQos(1);
